    ...
    try {
        ...
        if ($Incremental -or $Since) {
            ...
            if ($sinceRef) {
                ...
                $findings.Untested = @($findings.Untested | Where-Object { $_.SourceFile -in $changedFiles })
                $findings.UnderTested = @($findings.UnderTested | Where-Object { $_.SourceFile -in $changedFiles })
                $findings.TestlessDirs = @($findings.TestlessDirs | Where-Object { $_.Directory -in $changedFiles })
                $totalGaps = $findings.Untested.Count + $findings.UnderTested.Count + $findings.TestlessDirs.Count
                if ($totalGaps -eq 0) {
                    Write-Step "No coverage gaps in changed files — nothing to do" "OK"
